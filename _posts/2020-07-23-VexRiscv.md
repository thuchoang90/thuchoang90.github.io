...
$ export PATH=/opt/riscv/bin/:$PATH       #if not, then export the riscv32im toolchain to the PATH
...
Same procedure as [II. b) Debug Briey SoC with Verilator + OpenOCD + GDB](#iii-b-debug-briey-soc-with-verilator--openocd--gdb) with three separated terminals. Just replace the GDB-terminal step with this Eclipse step: the Eclipse tool prepared in [Fresh-Ubuntu-setup](/tutorial/2020/07/23/Fresh-Ubuntu-setup#h-ii-f-eclipse).
...
Same procedure as [II. b) Debug Briey SoC with Verilator + OpenOCD + GDB](#iii-b-debug-briey-soc-with-verilator--openocd--gdb) with three separated terminals. Just replace the Verilator-terminal step with a real hardware on an FPGA.
...
5. Run or debug software *(by GDB or Eclipse)*
  ...
  * Using Eclipse: the same as [II. c) Debug with Eclipse](#iii-c-debug-with-eclipse)
